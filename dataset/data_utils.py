...
def check_and_validate_polys(polys, xxx_todo_changeme):
    ...
    :return:
    '''
    polys = np.array([np.array(poly, dtype=object) for poly in polys], dtype=object)
    ...
    return np.array(validated_polys,dtype=object)


def generate_rbox(im_size, text_polys, text_tags, training_mask, i, n, m):
    """
    生成mask图，白色部分是文本，黑色是ackground
    ...
    for poly, tag in zip(text_polys, text_tags):
        ...
        if not shrinked_poly:
            shrinked_poly = np.array(shrinked_poly)[np.newaxis, :, :]
        else:
            shrinked_poly = np.array(shrinked_poly[0]).reshape(-1, 2)[np.newaxis, :, :]
...
def image_label(im_fn: str, text_polys: np.ndarray, text_tags: list, n: int, m: float, input_size: int,
                defrees: int = 10,
                scales: np.ndarray = np.array([0.5, 1, 2.0]), img_corners=None) -> tuple:
    '''
    ...
    h, w, _ = im.shape
    if isinstance(img_corners, list):
        im = im[img_corners[1]:img_corners[3], img_corners[0]:img_corners[2]]
        h, w, _ = im.shape
...
class TibetanDataset(data.Dataset):
    def __init__(self, data_dir, data_shape: 860, n=3, m=0.5, transform=None, target_transform=None,
                 use_sub=True, base_path=''):
        self.data_list = self.load_json(data_dir)
    ...
    def __getitem__(self, index):
        # print(self.image_list[index])
        if self.use_sub:
            img_inf = self.data_list["sub_images"][index]
            img_path, text_polys, text_tags, img_corners = img_inf['filename'], np.array(img_inf['polygon'],
                                                                                         dtype=object), list(
                np.zeros(
                    (len(img_inf['polygon']),), dtype=np.bool)), img_inf['corner']
        else:
            img_inf = self.data_list["images"][index]
            img_path, text_polys, text_tags = img_inf['filename'], np.array(img_inf['polygon'], dtype=object), list(
                np.zeros(
                    (len(img_inf['polygon']),), dtype=np.bool))
            img_corners = None
        img, score_maps, training_mask = image_label(self.base_path + 'img/' + img_path, text_polys, text_tags,
                                                     input_size=self.data_shape,
                                                     n=self.n,
                                                     m=self.m,
                                                     img_corners=img_corners)
        # score_maps = np.array(score_maps[0]).squeeze()
        # heatmap = (score_maps * 255).astype(np.uint8)
        # heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
        #
        # overlap = img.copy()
        # alpha = 0.4
        # cv2.rectangle(overlap, (0, 0), (img.shape[1], img.shape[0]), (255, 0, 0), -1)
        # cv2.addWeighted(overlap, alpha, img, 1 - alpha, 0, img)
        # cv2.addWeighted(heatmap, alpha, img, 1 - alpha, 0, img)
        # cv2.imshow("fig1", img)
        # cv2.waitKey(0)
        if self.transform:
            img = self.transform(img)
    ...
    def load_json(self, json_path):
        with open(json_path, 'r') as result_file:
            img_inf = json.load(result_file)
        return img_inf
    ...
    def __len__(self):
        if self.use_sub:
            return len(self.data_list["sub_images"])
        else:
            return len(self.data_list["images"])
