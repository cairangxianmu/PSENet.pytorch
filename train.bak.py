...
from dataset.data_utils import MyDataset, TibetanDataset
...
def train_epoch(net, optimizer, scheduler, train_loader, device, criterion, epoch, all_step, writer, logger):
    ...
    lr = scheduler.get_last_lr()[0]  # get_last_lr get_lr
        ...
        for img_name in tqdm(imgs_name, desc="test models"):
            ...
            if use_sub:
                ...
                for corner in filename_to_corner[img_name]:
                    ...
                    cv2.rectangle(img, (corner[0], corner[1]), (corner[2], corner[3]), (255, s, 0), 5)
                    s += 20
...
def main():
    ...
    train_data = TibetanDataset(config.json_path, data_shape=config.data_shape, n=config.n, m=config.m,
                                transform=transforms.ToTensor(), base_path=config.base_path)
